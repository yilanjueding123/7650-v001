...
INT8S ap_video_record_md_active(INT8U *md_tick, INT32U state)
{
    *md_tick = 0;
    ...
    if (!(video_record_sts & VIDEO_RECORD_BUSY))		//检测移动侦测，开始录像
    {
...
#define VEDIO_KEY_UP 			0
#define VEDIO_KEY_DOWN 			1

INT8U video_stop_flag = 0;
extern INT8U cyc_restart_flag;
volatile INT8U video_down_flag = VEDIO_KEY_UP;//0;
//extern INT8U  File_creat_ok;

//extern INT8U event_stop_flag;
extern volatile INT32U  current_state;
INT8S ap_video_record_func_key_active(INT32U event)
{
    ...
    video_down_flag = VEDIO_KEY_DOWN;//1;

    if (video_record_sts & VIDEO_RECORD_BUSY)
    {
        video_stop_flag = 1;
        if(ap_state_handling_file_creat_get())			//save file
        {
            //led_blue_on();
            DBG_PRINT("red_led\r\n");
        }
        if((event == MSG_APQ_VIDEO_RECORD_ACTIVE) || (event == MSG_APQ_CAPTUER_ACTIVE))
        {
            ...
            {
                video_down_flag = VEDIO_KEY_UP;//0;
                return STATUS_FAIL;
            }
            else
            {
...
#endif
        video_calculate_left_recording_time_enable();		//计算最后剩下的视频文件大小
        if (video_encode_stop() != START_OK)				//视频编码停止,关闭视频录像
        {
        ...
        else
        {
            ...
            {
                _setfattr((CHAR *) curr_file_info.file_path_addr, D_RDONLY);			//重新设置文件属性
            }
#if GPS_TXT
...
#if C_CYCLIC_VIDEO_RECORD == CUSTOM_ON
        if (cyclic_record_timerid != 0xFF)		//close cyclic record timer
        {
        ...
        if(ap_state_handling_storage_id_get() != NO_STORAGE)
        {
            if(!cyc_restart_flag && ap_state_handling_file_creat_get())
            {
                ...
                if((event != MSG_APQ_VDO_REC_STOP) && (event != MSG_APQ_POWER_KEY_ACTIVE))
                {
                    ...
                    {
                        if(current_state == STATE_VIDEO_MONDET)
                        {
                            led_type = LED_MOTION_END;
                        }
                        else
                        {
    ...
    {
        if ((video_record_sts & VIDEO_RECORD_UNMOUNT) == 0)			//有卡状态
        {
#if C_MOTION_DETECTION == CUSTOM_ON
...
void ap_video_record_reply_action(STOR_SERV_FILEINFO *file_info_ptr)
{
    ...
    INT8U temp[6] = {0, 1, 2, 3, 5, 20};//10};		//modify by zhibo, 20170515

    value = temp[ap_state_config_record_time_get()];

	__msg("ap_video_record_reply_action,value = %d\n", value);
    if (value == 0)
    {
    ...
    if (src.type_ID.FileHandle >= 0)
    {
        ...
        else
        {
...
#if C_AUTO_DEL_FILE == CUSTOM_ON
            {
                ...
                {
                    disk_free_size = vfsFreeSpace(MINI_DVR_STORAGE_TYPE) >> 20;
                    DBG_PRINT("[Bkground Del Detect (DskFree: %d MB)]\n", disk_free_size);
