...
void ap_state_handling_connect_to_pc(INT32U prev_state)
{
	...
	__msg("ap_state_handling_connect_to_pc\n");
	...
	{
		key_down = gpio_read_io(SWITCH_MODE_KEY);
		__msg("key_down = %d\n", key_down);
		if(key_down)
		{
			temp = 1;			
...
void ap_state_handling_power_off(void)
{
    ...
    rtc_int_set(GPX_RTC_HR_IEN | GPX_RTC_MIN_IEN | GPX_RTC_SEC_IEN | GPX_RTC_HALF_SEC_IEN,\
                (RTC_DIS & GPX_RTC_HR_IEN) | (RTC_DIS & GPX_RTC_MIN_IEN) | (RTC_DIS & GPX_RTC_SEC_IEN) | \
                (RTC_DIS & GPX_RTC_HALF_SEC_IEN));
