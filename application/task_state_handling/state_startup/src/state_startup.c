...
void state_startup_init(void)			//开机logo
{
#if C_LOGO == CUSTOM_ON
...
void state_startup_entry(void *para)
{
	...
	__msg("state_startup_entry\n");
    task_display_init();				//显示初始化相关
    audio_task_init();					//音频初始化
    task_peripheral_handling_init();	//外围设备初始化
    state_startup_init();				//启动状态初始化
