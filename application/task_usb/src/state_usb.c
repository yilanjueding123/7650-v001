...
static INT8U usb_state_flag = 0; //0:无USB 无SD 1:USB 2:SD 3:usb or SD
 ...
 * 负 责 人  : Zhibo
 * 创建日期  : 2017年5月4日
 * 函数功能  : usb states: 0 none; 1 usb; 2 sd; 3 sd and usb
 * 输入参数  : void  NULL
 * 输出参数  : 无
 * 返 回 值  : 
 * 调用关系  : 
...
static void state_usb_isr(void)
{
    ...
    if  ( rDLCIF_UDLC & MASK_USBD_UDLC_IF_RESET )  // Reset 
    {
        //drv_l1_usb_soft_disconnect();  //D+, D-
        usb_state_flag = 1;
