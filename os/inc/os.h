 ...
 * 负 责 人  : Zhibo
 * 创建日期  : 2017年4月28日
 * 函数功能  : OSQPend()函数用于任务等待消息。
 * 输入参数  : OS_EVENT *pevent  pevent
               INT16U timeout    timeout
               INT8U *err        err
 * 输出参数  : 无
 * 返 回 值  : void
 * 调用关系  : 
 * 其    它  : 消息通过中断或者另外的任务发送给需要的任务。消息是一个以指针定义的变量，
 			   在不同的程序中消息的使用也可能不同。如果调用OSQPend()函数时，
 			   队列中已经存在需要的消息，那么该消息被返回给OSQPend()函数的调用者，
 			   队列中清除该消息；若调用OSQPend()函数时，队列中没有需要的消息，
 			   那么OSQPend()函数挂起当前任务，直到得到需要的消息或者超出定义的操时时限。
 			   如果同时有多个任务等待同一个消息，那么μC/OS-Ⅱ系统默认最高优先级的任务取
 			   得消息，并且任务恢复执行。一个OSTaskSuspend()函数挂起的任务也可以接收消息，
 			   但这个任务一直保持挂起状态，直到通过调用OSTaskResume()函数恢复任务的运行。
 ...
 * 负 责 人  : Zhibo
 * 创建日期  : 2017年4月28日
 * 函数功能  : OSQIntPost()在中断或内核函数中使用，OSQIntPost()通过消息队列
               向任务发送消息。
 * 输入参数  : OS_EVENT *pevent  pevent
               void *msg         待发送的消息
 * 输出参数  : 无
 * 返 回 值  : 
 * 调用关系  : 
 * 其    它  : 消息是一个 uint8 整型数，在不同应用中有不同的含义。如果消息队列已经存满，
 			则返回错误码。如果有任何等待消息的任务在等待消息，则这些任务中的最高级任务
 			将得到消息。如果得到消息的任务比当前的任务优先级高，则在所有中断退出后或调用 
 			SSched()后，高优先级的任务将得到消息而恢复执行。消息队列是先入先出机制的，
 			先入队列消息先被传递给任务。
